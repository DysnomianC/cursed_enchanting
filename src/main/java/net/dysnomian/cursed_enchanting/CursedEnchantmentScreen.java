...
public class CursedEnchantmentScreen extends HandledScreen<CursedEnchantmentScreenHandler> {
   ...
   private ItemStack stack = ItemStack.EMPTY;
	...
	public CursedEnchantmentScreen(CursedEnchantmentScreenHandler handler, PlayerInventory inventory, Text title) {
   ...
   protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
      ...
      int n = ((CursedEnchantmentScreenHandler)this.handler).getRedstoneCount();
		...
		for(int o = 0; o < 3; ++o) {
			...
			this.setZOffset(0);
			this.client.getTextureManager().bindTexture(TEXTURE);
			int r = ((CursedEnchantmentScreenHandler)this.handler).enchantmentPower[o];
			RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
			if (r == 0) {
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		for(int j = 0; j < 3; ++j) {
			...
			Enchantment enchantment = Enchantment.byRawId(((CursedEnchantmentScreenHandler)this.handler).enchantmentId[j]);
			int l = ((CursedEnchantmentScreenHandler)this.handler).enchantmentLevel[j];
			int m = j + 1;
			if (this.isPointWithinBounds(60, 14 + 19 * j, 108, 17, (double)mouseX, (double)mouseY) && k > 0 && l >= 0 && enchantment != null) {
				...
				if (!bl) {
					...
					if (this.client.player.experienceLevel < k) {
						list.add((new TranslatableText("container.enchant.level.requirement", new Object[]{((CursedEnchantmentScreenHandler)this.handler).enchantmentPower[j]})).formatted(Formatting.RED));
					} else {
   ...
   public void doTick() {
		ItemStack itemStack = ((CursedEnchantmentScreenHandler)this.handler).getSlot(0).getStack();
		...
		for(int i = 0; i < 3; ++i) {
			if (((CursedEnchantmentScreenHandler)this.handler).enchantmentPower[i] != 0) {
			bl = true;
			}
		}
